...
def create_prompt():
    country = input("What country would you like a news summary for? ")  # Get Country
    try:
    ...
    # Grab all the text
    country_headlines = ''
    for item in soup.select(tag)[:3]:  # Grabs top 3 tags
        country_headlines += item.getText() + '\n'

    prompt_text = "Detect the language of the news headlines below, then translate a summary of the headlines to English in a conversational tone:\n"
    return prompt_text + country_headlines
...
if __name__ == "__main__":
    ...
    translate_and_summary(prompt_text)
